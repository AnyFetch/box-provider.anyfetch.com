...
module.exports = function upload(file, anyfetchClient, accessToken, cb) {
  ...
  async.waterfall([
    ...
    function handleMetada(metadata, cb) {
      ...
      document.identifier = metadata.id;
      document.actions.show =  'https://app.box.com/files/0/f/' + metadata.parent.id + '/1/f_' + metadata.id;
      // do not download the file when it is scheduled for deletion or if it is bigger than 50Mb
      if(!metadata.trashed_at && metadata.size < 52428800 && new Date(metadata.modified_at).getTime() > file.cursor) {
        ...
        cb(new Error('File was already uploaded'));
      }
    },
    ...
    function getFileLocation(res, cb) {
