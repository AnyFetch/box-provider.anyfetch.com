  ...
  async.waterfall([
    ...
    function handleMetada(metadata, cb) {
      ...
      if(serviceData.next_stream_position && metadata.body.entries.length > 0) {
        ...
        var stringEntries = [];
        metadata.body.entries.forEach(function(item) {
          stringEntries.push(JSON.stringify(item));
        });
        toUnique(stringEntries);
        metadata.body.entries = [];
        stringEntries.forEach(function(item) {
          metadata.body.entries.push(JSON.parse(item));
        });
        // handling new files
